 ...
 */
public class DefaultClassloadingDelegate<T> implements ClassloadingDelegate<T> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultClassloadingDelegate.class);

    protected Map<String, Class<? extends T>> supported;

    public DefaultClassloadingDelegate(Class<? extends T>... classes) {
        Map<String, Class<? extends T>> supported = Maps.newHashMap();
    ...
    @Override
    public Class<? extends T> load(String classname) throws ClassNotFoundException {
