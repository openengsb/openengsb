...
/**
 * The Endpoint to the update-feature
 */
public class UpdateEndpoint extends AbstractScmEndpoint {
    private static final String BEHAVIOR = "updating working-copy.";

    @Override
    protected void processInOutRequest(MessageExchange exchange, NormalizedMessage in, NormalizedMessage out)
            throws Exception {
        // get parameters
        String updatePath = extractStringParameter(in, "./@" + ParameterNames.UPDATE_PATH);

        // log parameters
        if (getLog().isDebugEnabled()) {
            StringBuilder parameters = new StringBuilder();
            parameters.append("Parameters:\n");
            parameters.append("updatePath=");
            parameters.append(updatePath);
        ...
        // execute call
        Command<MergeResult> command = getCommandFactory().getUpdateCommand(updatePath);
    ...
    protected String getEndpointBehaviour() {
        return UpdateEndpoint.BEHAVIOR;
    }
}
