...
public class FakeComponentsRetriever implements ComponentsRetriever
{
	...
	public List<ComponentDescriptor> lookupComponents() {
		...
		for (String s : toread) {
			try {
				ComponentDescriptor jbi = parser.parseJbi(new InputSource(context
						.getResourceAsStream("/descriptors/" + s + ".xml")));
				ComponentDescriptor schema = parser.parseSchema(new InputSource(context
						.getResourceAsStream("/descriptors/" + s + ".xsd")));
				components.add(new ComponentDescriptor(jbi.getType(), jbi.getName(),
						jbi.getDescription(), schema.getTargetNamespace(), schema.getEndpoints()));
			} catch (Exception e) {
