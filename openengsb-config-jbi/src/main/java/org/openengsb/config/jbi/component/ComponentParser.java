...
public class ComponentParser
{
	...
	public ComponentDescriptor parseJbi(InputSource in) throws ParseException {
		...
		try {
			...
			Object componentNode = xpath.evaluate("/jbi:jbi/jbi:component", doc
	...
	private ArrayList<EndpointDescriptor> parseEndpoints(Element root, XPath xpath)
			throws ParseException {
		ArrayList<EndpointDescriptor> endpoints = new ArrayList<EndpointDescriptor>();
		try {
			NodeList elementNodes = (NodeList) xpath.evaluate("/xsd:schema/xsd:element", root,
			...
			for (int i = 0; i < elementNodes.getLength(); ++i) {
				...
				endpoints.add(new EndpointDescriptor(elem.getAttribute("name"), parseAttributes(
						elem, xpath)));
			}
		} catch (XPathExpressionException e) {
			throw new ParseException(e);
	...
	private ArrayList<AttributeDescriptor> parseAttributes(Element endpoint, XPath xpath)
		...
		NodeList attributeNodes = (NodeList) xpath.evaluate("./xsd:complexType/xsd:attribute",
				endpoint, XPathConstants.NODESET);
		for (int i = 0; i < attributeNodes.getLength(); ++i) {
			Element attribute = (Element) attributeNodes.item(i);
			attributes.add(new AttributeDescriptor(attribute.getAttribute("name"), attribute
					.getAttribute("type")));
		}
		return attributes;
	}
