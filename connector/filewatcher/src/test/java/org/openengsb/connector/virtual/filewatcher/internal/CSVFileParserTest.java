...
public class CSVFileParserTest {
    ...
    private FileSerializer<TestModel> parser = new CSVParser<TestModel>(TestModel.class);
    private File testcsv;

    @Before
    public void setUp() throws Exception {
        testcsv = temporaryFolder.newFile("test.csv");
    }

    @Test
    public void testParseCSVShouldCreateObject() throws Exception {
        FileUtils.write(testcsv, ""
                + "42,\"foo\", 7\n"
                + "21,\"bar\", 9\n");
        List<TestModel> testModels = parser.readFile(testcsv);
    ...
    @Test
    public void testWriteCSVAndParseShouldBeTheSame() throws Exception {
        List<TestModel> models = Arrays.asList(
            new TestModel(42, "foo", 7L),
            new TestModel(21, "bar", 9L)
        );
        parser.writeFile(testcsv, models);
        List<TestModel> parsedModels = parser.readFile(testcsv);
