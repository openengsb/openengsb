...
public class WorkflowServiceImpl implements WorkflowService, BundleContextAware, ServiceListener {
    ...
    @Override
    public void registerFlowTriggerEvent(Event event, String flowId) throws WorkflowException {
        String eventName = event.getName();
        String ruleCode = "## This rule has been autogenerated by the WorkflowService\n" +
                "when\n" +
                String.format("  Event ( name == \"%s\" )\n", eventName) +
                "then\n" +
                String.format("  flowHelper.startFlow(\"%s\");", flowId);
        String ruleName = String.format("_generated_ trigger %s on %s", flowId, eventName);
        log.info("adding new rule with id: " + ruleName);
        try {
            rulemanager.add(new RuleBaseElementId(RuleBaseElementType.Rule, ruleName), ruleCode);
