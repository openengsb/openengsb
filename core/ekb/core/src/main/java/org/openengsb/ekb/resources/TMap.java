    ...
    public TMap(URL tmapLocation) throws TMapException {
        ...
        try {
            ...
            try {
                ...
                StreamSource ssource = new StreamSource(TMap.class
                        .getResourceAsStream(TMAPSCHEMA));
                Schema schema = factory.newSchema(ssource);
                Validator validator = schema.newValidator();
                validator.validate(new DocumentSource((Node) doc));
            } catch (SAXException e) {
                logger.error("Error validating TMap: "
                        + tmapLocation.toExternalForm(), e);
                throw new TMapException("Error validating TMap: "
                        + tmapLocation.toExternalForm() + ". " + e.getMessage());
            } catch (IOException e) {
                logger.error("Error reading Tmap: "
                        + tmapLocation.toExternalForm(), e);
                throw new TMapException("Error reading Tmap: "
                        + tmapLocation.toExternalForm() + ". " + e.getMessage());
            }

            logger.debug("Loading output message of the T-Map...");
            Element output = (Element) doc
                    .selectSingleNode("//tmap/outputMessage");
            ...
            List<Element> segments = doc
                    .selectNodes("//tmap/outputMessage/segment");
            for (Element segmentElement : segments) {
                String segmentName = segmentElement.attributeValue("name");
                String domainConcept = segmentElement
                        .attributeValue("domainConcept");
                String format = segmentElement.attributeValue("format");
                outputSegments.add(new OpenEngSBMessageSegment(segmentName, domainConcept,
                        format, null));
            }
            this.outputMessage = new OpenEngSBMessage(receiverMessageType,
                    outputSegments);

            logger.debug("Loading input message of the T-Map...");
            Element input = (Element) doc
                    .selectSingleNode("//tmap/inputMessage");
            ...
            for (Element segmentElement : segments) {
                ...
                String domainConcept = segmentElement
                        .attributeValue("domainConcept");
                String format = segmentElement.attributeValue("format");
                inputSegments.add(new OpenEngSBMessageSegment(segmentName, domainConcept,
                        format, null));
            }
            this.inputMessage = new OpenEngSBMessage(senderMessageType,
                    inputSegments);
            ...
            for (Element converterElement : converters) {
                String inputFormat = converterElement
                        .attributeValue("inputFormat");
                String outputFormat = converterElement
                        .attributeValue("outputFormat");
            ...
            logger.error("Error loading T-Map! No valid url for T-Map: "
                    + tmapLocation, e);
            ...
            logger.error("Error loading T-Map! T-Map is not a valid XML file: "
                    + tmapLocation, e);
            throw new TMapException(e.getMessage());
        }
    }

    public OpenEngSBMessage performTransformation(OpenEngSBMessage inputMessage)
        ...
        for (OpenEngSBMessageSegment inputSegment : inputMessage.getPayload()) {
            ...
            for (OpenEngSBMessageSegment outputSegment : this.getOutputMessage()
                    .getPayload()) {
                ...
                if (inputDomainConcept.equals(outputSegment.getDomainConcept())
                    ...
                    outputSegments.add(new OpenEngSBMessageSegment(outputSegment
                            .getSegmentName(),
                            outputSegment.getDomainConcept(), outputSegment
                            .getFormat(), inputSegmentContent));
                    break;
                }
                // if domainConcept is the same, but format is different a
                // converter needs to be called
                else if (inputDomainConcept.equals(outputSegment
                        .getDomainConcept())) {
                    if (this.getConverters().get(inputFormat) == null
                            || this.getConverters().get(inputFormat).get(
                            outputSegment.getFormat()) == null) {
                        logger
                                .error("Error during Transformation: No suitable Converter available!");
                        throw new TMapException(
                                "Error during Transformation: No suitable Converter available!");
                    }
                    try {
                        outputSegments.add(new OpenEngSBMessageSegment(
                                outputSegment.getSegmentName(), outputSegment
                                .getDomainConcept(), outputSegment
                                .getFormat(), this.getConverters().get(
                                inputFormat).get(
                                outputSegment.getFormat()).convert(
                                inputSegmentContent).toString()));
                    } catch (ConversionException ex) {
                        logger.error("Error during Transformation", ex);
                        throw new TMapException("Error during Transformation: "
                                + ex.getMessage());
                    }
                }
            }
        ...
        OpenEngSBMessage outputMessage = new OpenEngSBMessage(inputMessage
                .getMessageID(), inputMessage.getCreated(), inputMessage
                .getSender(), inputMessage.getReceiver(), inputMessage
                .getMessageType(), outputSegments);
