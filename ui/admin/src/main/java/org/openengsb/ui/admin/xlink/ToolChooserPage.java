...
public class ToolChooserPage extends WebPage{
    ...
    public void preProcessingPage(){
        ...
        if(checkForLocalSwitchingParameters()){
            ...
            XLinkMock.transformAndOpenMatch(sourceModelClass, versionId, identifierValues, destinationModelClass.getClassName(), destinationModelClass.getVersion(), connectorId, viewId);
            ...
            protected void populateItem(ListItem item) {
                ...
                ListView viewList = new ListView("viewList", tool.getAvailableViews()) {
                    ...
                    protected void populateItem(ListItem li) {
                        ...
                        final XLinkModelInformation destModelInfo = XLinkMock.getModelClass(tool.getId().toFullID(), view.getViewId());
                        Link viewLink = new Link("viewLink") {
                            public void onClick() {
                                String sourceModelClass = modelId;   
                                XLinkMock.transformAndOpenMatch(sourceModelClass, versionId, identifierValues, destModelInfo.getClassName(), destModelInfo.getVersion(), tool.getId().toFullID(), view.getViewId());
                                handleSuccessResponse(resp);
                            }
                        };
                        if(XLinkMock.isTransformationPossible(modelId, versionId, destModelInfo.getClassName(), destModelInfo.getVersion())){
